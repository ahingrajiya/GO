...
private:
  ...
  void initTree(const edm::Service<TFileService> &fs);
  ...
  // float towerphi[9999], towereta[9999], towerpt[9999], toweret[9999], towerenergy[9999], towerlead[9999];
  float towerphi, towereta, towerpt, toweret, towerenergy, towerlead;
...
void DemoAnalyzer::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup)
{
  ...
  ev = ev + 1;
  if (HFfilter->numMinHFTowers4 < 2)
  {
    evfail = evfail + 1;
    cout << "Event fail: " << evNumber << endl;
    evn = evNumber;
  }
  else
  {
  ...
  for (unsigned it = 0; it < tracks->size(); ++it)
  {
    ...
    if (tk.pdgId() == 1 || tk.pdgId() == 2)
    {
      ...
      if (eta_plus || eta_minus)
      {
        ...
        if (tk.pdgId() == 1)
          tower_ethad->Fill(tk.et());
        if (tk.pdgId() == 2)
          tower_etem->Fill(tk.et());
        count = count + 1;
      }
      if (eta_minus)
      {
...
void DemoAnalyzer::initTree(const edm::Service<TFileService> &fs)
{
  hitree = fs->make<TTree>("HiTree", "");
  hitree->Branch("towerphi", &towerphi, "towerphi/F");
  hitree->Branch("towerpt", &towerpt, "towerpt/F");
  hitree->Branch("towereta", &towereta, "towereta/F");
  hitree->Branch("count", &count, "count/I");
  hitree->Branch("toweret", &toweret, "toweret/F");
  hitree->Branch("towerenergy", &towerenergy, "towerenergy/F");
}
