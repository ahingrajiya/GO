...
private:
  ...
  void hbt(std::vector<TLorentzVector> GoodTrackFourVector,
  ...
  void MixEvents(int ntrkoff_min,
                 int ntrkoff_max,
                 int nEvt_to_mix,
                 std::vector<int> ev_ntrkoff,
                 std::vector<double> pv_vtx_z,
                 double vzcut,
                 std::vector<std::vector<TLorentzVector>> ev_GoodTrackFourVector,
                 std::vector<std::vector<int>> ev_GoodTrackCharge,
                 THnSparse* h_ss,
                 THnSparse* h_os);
...
DemoAnalyzer::DemoAnalyzer(const edm::ParameterSet& iConfig)
    : vertexToken_(consumes<reco::VertexCollection>(iConfig.getParameter<edm::InputTag>("vertexCollection"))),
      ...
      pfCandSrc_(consumes<reco::PFCandidateCollection>(iConfig.getParameter<edm::InputTag>("pfSrc"))) {
...
void DemoAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (size_t i = 0; i < towers->size(); ++i) {
    ...
    if (eta_plus || eta_minus) {
      ...
      if (tk.energy() >= leading_energy) {
        leading_energy = tk.energy();
      }
      tower_ethad->Fill(tk.hadEt());
      tower_etem->Fill(tk.emEt());
      count = count + 1;
    }
    if (eta_minus) {
  ...
  unsigned short int countpf = 0;
  unsigned short int count_TH4_pluspf = 0;
  unsigned short int count_TH4_minuspf = 0;
  float leading_PFenergy = 0;
  for (size_t ic = 0; ic < pfCandidates->size(); ++ic) {
    ...
    // if(!(type == PFCandidate::h || type == PFCandidate::e || type == PFCandidate::mu)) continue;
    const bool eta_pluspf = (cand.eta() > 3.0) && (cand.eta() < 6.0);
    const bool eta_minuspf = (cand.eta() < -3.0) && (cand.eta() > -6.0);
    if (cand.et() < 0.0)
      continue;
    if (eta_pluspf || eta_minuspf) {
      pf_pt->Fill(cand.pt());
      pf_phi->Fill(cand.phi());
      pf_eta->Fill(cand.eta());
      pf_et->Fill(cand.et());
      pf_ethad->Fill(cand.ecalEnergy());
      pf_etem->Fill(cand.hcalEnergy());
      pf_energy->Fill(cand.energy());
      if (cand.energy() >= leading_PFenergy) {
        leading_PFenergy = cand.energy();
      }
      countpf = countpf + 1;
    }
    if (eta_minuspf) {
      count_TH4_minuspf += cand.energy() >= 4.0 ? 1 : 0;
    } else if (eta_pluspf) {
      count_TH4_pluspf += cand.energy() >= 4.0 ? 1 : 0;
    }
  }
  tower_energy_lead->Fill(leading_energy);
  tower_energy_PFlead->Fill(leading_PFenergy);

  Ntowers->Fill(count);
  Ntowers_PF->Fill(countpf);
  ...
  MinNtower_TH4GeV_PF->Fill(std::min(count_TH4_pluspf, count_TH4_minuspf));

  if (std::min(count_TH4_plus, count_TH4_minus) < 2) {
    evfaily = evfaily + 1;
  } else {
    evpassy = evpassy + 1;
  }
...
//      GoodTrackCharge.push_back(iter_tk->charge());
      ...
      chi2_reco->Fill(iter_tk->normalizedChi2()/iter_tk->hitPattern().trackerLayersWithMeasurement());
      ...
      if(iter_tk->hitPattern().pixelLayersWithMeasurement()>0){GoodTrackFourVector.push_back(pvector); GoodTrackCharge.push_back(iter_tk->charge());}
      ...
      if(iter_tk->hitPattern().pixelLayersWithMeasurement()>0){GoodTrackFourVector_corr.push_back(pvector); GoodTrackCharge_corr.push_back(iter_tk->charge());}
      ...
      chi2_reco_corr->Fill(iter_tk->normalizedChi2()/iter_tk->hitPattern().trackerLayersWithMeasurement());
      if(iter_tk->charge()>0){
...
void DemoAnalyzer::initHistos(const edm::Service<TFileService>& fs) {
  ...
  tower_energy_PFlead = Inf.make<TH1F>("tower_energy_PFlead", "", 1000, 0.0, 100.0);
  ...
  Ntowers_PF = Inf.make<TH1F>("Ntowers_PF", "", 1000, 0.0, 1000.0);
  pf_pt = Inf.make<TH1F>("pf_pt", "", 1000, 0.0, 100.0);
  pf_eta = Inf.make<TH1F>("pf_eta", "", 200, -6.0, 6.0);
  pf_phi = Inf.make<TH1F>("pf_phi", "", 100, -3.2, 3.2);
  pf_et = Inf.make<TH1F>("pf_et", "", 1000, 0.0, 100.0);
  pf_energy = Inf.make<TH1F>("pf_energy", "", 1000, 0.0, 100.0);
  pf_ethad = Inf.make<TH1F>("pf_ethad", "", 1000, 0.0, 100.0);
  pf_etem = Inf.make<TH1F>("pf_etem", "", 1000, 0.0, 100.0);
  MinNtower_TH4GeV_PF = Inf.make<TH1F>("MinNtower_TH4GeV_PF", "", 1000, 0.0, 1000.0);
}
...
void DemoAnalyzer::hbt(std::vector<TLorentzVector> GoodTrackFourVector,
                       std::vector<int> GoodTrackCharge,
                       Double_t cc,
                       THnSparse* h_ss,
                       THnSparse* h_os,
  ...
  for (unsigned int itk1 = 0; itk1 < GoodTrackFourVector.size(); itk1++) {
    ...
    for (unsigned int itk2 = itk1 + 1; itk2 < GoodTrackFourVector.size(); itk2++) {
      ...
      if (splitcomb(GoodTrackFourVector[itk1], GoodTrackFourVector[itk2], cosdpt))
        continue;

      if (charge1 * charge2 > 0) {
        h_ss->Fill(x3D);
        h_ss_rot->Fill(x3D_rot);
      } else {
        h_os->Fill(x3D);
        h_os_rot->Fill(x3D_rot);
      }
    }
  }
}

void DemoAnalyzer::MixEvents(int ntrkoff_min,
                             int ntrkoff_max,
                             int nEvt_to_mix,
                             std::vector<int> ev_ntrkoff,
                             std::vector<double> pv_vtx_z,
  ...
  for (int nevt1 = 0; nevt1 < aux_n_evts; nevt1++) {
    ...
    if (ev_ntrkoff[nevt1] < ntrkoff_min || ntrkoff_max < ev_ntrkoff[nevt1])
      continue;
    int takeAssociated = 0;
    for (int nevt_assoc = nevt1 + 1; nevt_assoc < aux_n_evts; nevt_assoc++) {
      if (ev_ntrkoff[nevt_assoc] < ntrkoff_min || ntrkoff_max < ev_ntrkoff[nevt_assoc])
        continue;
      if (fabs(pv_vtx_z[nevt1] - pv_vtx_z[nevt_assoc]) > vzcut)
        continue;
      ...
      takeAssociated++;
      if (takeAssociated > nEvt_to_mix)
        break;  //for "nEvt_to_mix" events mixing. If you remove this "break", you will mix all the events.
      for (int imix = 0; imix < nMixmult_nevt1; imix++) {
        for (int iimix = 0; iimix < nMixmult_nevt_assoc; iimix++) {
          Double_t qmix =
              GetQ(ev_GoodTrackFourVectorTemp_nevt1_vec[imix], ev_GoodTrackFourVectorTemp_nevt_assoc_vec[iimix]);
          TLorentzVector psum2mix =
              ev_GoodTrackFourVectorTemp_nevt1_vec[imix] + ev_GoodTrackFourVectorTemp_nevt_assoc_vec[iimix];
          Double_t ktmix = (psum2mix.Pt()) / 2.;
          Double_t xmix3D[3] = {qmix, ktmix, (Double_t)ev_ntrkoff[nevt1]};
          if (ev_GoodTrackCharge_nevt1_vec[imix] * ev_GoodTrackCharge_nevt_assoc_vec[iimix] > 0) {
            h_ss->Fill(xmix3D);
          } else {
            h_os->Fill(xmix3D);
          }
...
bool DemoAnalyzer::splitcomb(TLorentzVector& vec1, TLorentzVector& vec2, TH2F* cosdpt) {
  bool issplit = false;
  Double_t cosa =
      TMath::Abs(vec1.Px() * vec2.Px() + vec1.Py() * vec2.Py() + vec1.Pz() * vec2.Pz()) / (vec1.P() * vec2.P());
